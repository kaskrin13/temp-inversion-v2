...
def tempInv(data):
    ...
    if currentTime.time() < datetime.time(12):  	
        if currentTemp - lowTemp[0] > 3:
            ...
            return (False, currentTemp, currentTime, currentWindSpeed, lowTemp[0], lowTemp[1], highTemp[0], highTemp[1])
        else:
            if (currentTemp - lowTemp[0]) < 2:
                # strong inversion and no spray suggested
                return (True, currentTemp, currentTime, currentWindSpeed, lowTemp[0], lowTemp[1], highTemp[0], highTemp[1])
            else:
...
# prints the results of the tempInv function in a readible format for error checking
# input: result = 8-tuple
def printResult(result):
    if result[0]:
        print("| {0:^10} | {1:^15} | {2:^20} | {3:^20} | {4:^15} | {5:^20} | {6:^15} | {7:^20} |".format("Yes", result[1], str(result[2]), result[3], result[4], str(result[5]), result[6], str(result[7])))
    else:
        print("| {0:^10} | {1:^15} | {2:^20} | {3:^20} | {4:^15} | {5:^20} | {6:^15} | {7:^20} |".format("No", result[1], str(result[2]), result[3], result[4], str(result[5]), result[6], str(result[7])))
...
def main():
    ...
    '''
    print("| {0:^10} | {1:^15} | {2:^20} | {3:^20} | {4:^15} | {5:^20} | {6:^15} | {7:^20} |".format("Inversion", "Current Temp", "Current Time", "Current Windspeed", "Low Temp", "Time of Low", "High Temp", "Time of High"))
    print("-"*160)
